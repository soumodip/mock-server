...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!integrationId || !sessionId) {
      throw createError({
        statusCode: 400,
        message: 'Integration ID and Session ID are required',
      });
    }
    ...
    // Build the API path (remove sdk prefix)
    const apiPath = '/' + (path || '');
    ...
    // Sort APIs so exact matches come first, then by specificity (fewer params = more specific)
    const matchingApis = allApis
      ...
      .map((a: any) => ({
        api: a,
        match: matchRoute(a.endpoint, apiPath),
      }))
      .filter((item: any) => item.match !== null)
      .sort((a: any, b: any) => {
        // Exact matches first
        if (a.api.endpoint === apiPath && b.api.endpoint !== apiPath) return -1;
        if (b.api.endpoint === apiPath && a.api.endpoint !== apiPath) return 1;
    ...
    if (!api) {
      throw createError({
        statusCode: 404,
        message: `No API found for ${method} ${apiPath}`,
      });
    }
    ...
    if (!activeStatusMock) {
      throw createError({
        statusCode: 500,
        message: `No active status mock configured for ${method} ${apiPath}`,
      });
    }
    ...
    console.error('Error serving SDK API:', error);
    throw createError({
      statusCode: error instanceof Error && 'statusCode' in error ? (error as any).statusCode : 500,
      message: error instanceof Error ? error.message : 'Failed to serve API',
    });
  }
});
