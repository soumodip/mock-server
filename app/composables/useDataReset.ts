...
// ── Shared singleton state (module-level, survives across component instances) ──
...
// Singleton polling guard — ensures only ONE set of intervals runs
// even when multiple pages/components call useDataReset()
let activeSubscribers = 0;
let pollInterval: ReturnType<typeof setInterval> | null = null;
let countdownInterval: ReturnType<typeof setInterval> | null = null;

// Per-project deletion dedup (prevents duplicate client-side check calls)
const pendingDeletionCheck = new Set<string>();
...
export function useDataReset() {
  ...
  const checkAndDeleteProject = async (projectId: string) => {
    if (pendingDeletionCheck.has(projectId)) {
      return;
    }

    pendingDeletionCheck.add(projectId);
    ...
    try {
      ...
      pendingDeletionCheck.delete(projectId);
    }
  };
  ...
  // ── Singleton polling: start only when first subscriber mounts ──
  const startPolling = () => {
    activeSubscribers++;

    // Only start intervals if this is the first subscriber
  ...
  // ── Stop polling only when the last subscriber unmounts ──
  const stopPolling = () => {
  ...
  // Clear the deleted project ID (called when modal is dismissed)
