<template>
  <div>
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-lg font-medium text-gray-200">Nuxt.js Frontend</h2>
...
onMounted(async () => {
  try {
    const response = await fetch('/code/frontend-nuxtjs.md');
...
const generateProjectMarkdown = (): any => {
  let md = `# Nuxt.js Frontend Implementation\n\n`;

  // Context
  md += `## Context\n`;
  md += `You are an experienced frontend developer working on a Nuxt-based web application. This is a state-aware implementation where each page is built incrementally based on provided specifications.\n\n`;
  ...
  md += `A working Nuxt.js project code is available in the file \`code.md\` (located in the same directory as this file). This code is production-ready and contains a fully functional implementation with all dependencies, configurations, and best practices already set up.\n\n`;
  ...
  md += `1. Create a \`.env\` file in your project root\n`;
  md += `2. Add the following environment variable:\n`;
  md += `   \`\`\`\n`;
  if (selectedProject.value?.id) {
    md += `   NUXT_PUBLIC_API_BASE_URL=${mockServerBaseUri}/api/projects/${selectedProject.value.id}\n`;
  } else {
    md += `   NUXT_PUBLIC_API_BASE_URL=${mockServerBaseUri}/api/projects/YOUR_PROJECT_ID\n`;
  }
  md += `   \`\`\`\n`;
  md += `3. Access it in your \`nuxt.config.ts\`:\n`;
  md += `   \`\`\`typescript\n`;
  md += `   export default defineNuxtConfig({\n`;
  md += `     runtimeConfig: {\n`;
  md += `       public: {\n`;
  if (selectedProject.value?.id) {
    md += `         apiBaseUrl: process.env.NUXT_PUBLIC_API_BASE_URL || '${mockServerBaseUri}/api/projects/${selectedProject.value.id}'\n`;
  } else {
    md += `         apiBaseUrl: process.env.NUXT_PUBLIC_API_BASE_URL || '${mockServerBaseUri}/api/projects/YOUR_PROJECT_ID'\n`;
  }
  md += `       }\n`;
  md += `     }\n`;
  md += `   });\n`;
  md += `   \`\`\`\n`;
  md += `4. Use it in your API client/composables:\n`;
  md += `   \`\`\`typescript\n`;
  md += `   const config = useRuntimeConfig();\n`;
  md += `   const baseURL = config.public.apiBaseUrl;\n`;
  md += `   \`\`\`\n\n`;

  // Project Overview
  md += `## Project Overview\n`;
  md += `- **Framework**: Nuxt 4 (Composition API, \`<script setup>\`, SSR enabled)\n`;
  md += `- **Vue**: 3.x with TypeScript strict mode\n`;
  md += `- **Styling**: Tailwind CSS (NO custom CSS - use utility classes only)\n`;
  md += `- **State Management**: Pinia (store modules under \`stores/*\`)\n`;
  md += `- **HTTP Client**: Axios for all API requests\n`;
  md += `- **Validation**: [SPECIFY: zod/vee-validate/yup]\n`;
  md += `- **UI Kit**: [SPECIFY: e.g., Tailwind + shadcn-vue/Nuxt UI/PrimeVue]\n`;
  md += `- **Routing**: File-based under \`pages/*\` (analyze reference repo for page structure)\n`;
  md += `- **Package Manager**: pnpm/npm/yarn\n\n`;

  // Requirements
  md += `## Requirements\n\n`;

  md += `### Code Quality Standards\n`;
  md += `- Use Composition API with \`<script setup>\` syntax exclusively\n`;
  md += `- Ensure type safety throughout with TypeScript strict mode\n`;
  md += `- Implement proper error handling and loading states\n`;
  md += `- Follow Vue 3 and Nuxt 4 best practices\n`;
  md += `- Use Pinia for state management with proper store patterns\n`;
  md += `- Implement proper composables for shared logic\n`;
  ...
  md += `**Modal Component Pattern (Vue/Nuxt):**\n`;
  md += `- When you think a modal needs to be created for a functionality, create it inside \`components/modals/\` folder\n`;
  md += `- Open modals inside pages using reactive state:\n`;
  md += `  \`\`\`vue\n`;
  md += `  <` + `script setup>\n`;
  md += `  const modal = ref({ type: null, data: null });\n`;
  md += `  const openModal = (type, data = null) => { modal.value = { type, data }; };\n`;
  md += `  const closeModal = () => { modal.value = { type: null, data: null }; };\n`;
  md += `  </` + `script>\n`;
  md += `  \n`;
  md += `  <` + `template>\n`;
  md += `    <CreateModal v-if="modal.value.type === 'create'" @close="closeModal" />\n`;
  md += `    <EditModal v-if="modal.value.type === 'edit'" :data="modal.value.data" @close="closeModal" />\n`;
  md += `  </` + `template>\n`;
  md += `  \`\`\`\n\n`;

  md += `### Technical Specifications\n`;
  md += `- Use Axios for all HTTP requests with proper error handling\n`;
  md += `- Implement proper TypeScript interfaces for all data structures\n`;
  md += `- Follow file-based routing conventions (analyze \`pages/*\` in reference repo)\n`;
  md += `- Use composables for shared business logic\n`;
  md += `- Implement proper SEO meta tags for each page\n`;
  md += `- Add proper loading and error states\n`;
  md += `- Review \`nuxt.config.ts\` and \`tailwind.config.js\` from the reference project\n\n`;
  ...
  md += `- [ ] Study the project structure, especially \`app/\` components and \`pages/\` organization\n`;
  md += `- [ ] Review configuration files (\`nuxt.config.ts\`, \`tailwind.config.js\`, \`tsconfig.json\`)\n`;
  md += `- [ ] Install dependencies using the appropriate package manager\n`;
  md += `- [ ] Verify the development server runs successfully\n`;
  md += `- [ ] Configure Pinia stores for your specific needs\n`;
  md += `- [ ] Setup Axios interceptors and API configuration\n\n`;
  ...
  if (components.value.length > 0) {
    ...
    components.value.forEach((comp, index) => {
      ...
      md += `- [ ] **${index + 1}.** \`components/${componentName}.vue\` (ref: \`components/${componentName}.md\`)${marker}\n`;
    });
    md += `\n`;
  }
  ...
  if (pages.value.length > 0) {
    ...
    pages.value.forEach((page, index) => {
      ...
      md += `- [ ] **${index + 1}.** \`pages/${fileName}.vue\` (ref: \`pages/${fileName}.md\`)${marker}\n`;
      if (apiCount > 0) {
  ...
  md += `- [ ] Run TypeScript type checking (\`npx nuxi typecheck\`)\n`;
  md += `- [ ] Final visual comparison against all screenshots\n\n`;

  // Deliverables
  md += `## Deliverables\n`;
  md += `- Fully functional Nuxt 4 application with all specified pages\n`;
  md += `- Proper integration with provided API endpoints\n`;
  md += `- Pinia stores for state management\n`;
...
const downloadZip = async () => {
  ...
  const projectName = 'frontend-nuxtjs';
